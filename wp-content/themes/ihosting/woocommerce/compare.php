...
<head>
	...
	<link rel="stylesheet" href="<?php echo ihosting_fonts_url(); ?>"/>
...
<!-- START BODY -->
<body <?php body_class( 'woocommerce compare-template' ) ?>>

<h1>
	<?php echo $localized_table_text ?>
	<?php if ( !$is_iframe ) : ?><a class="close"
	                                href="#"><?php _e( 'Close window [X]', 'ihosting' ) ?></a><?php endif; ?>
</h1>
...
<table class="compare-list" cellpadding="0" cellspacing="0"<?php if ( empty( $products ) )
	...
	<?php if ( empty( $products ) ) : ?>
		...
		<tr class="no-products">
			<td><?php _e( 'No products added in the compare table.', 'ihosting' ) ?></td>
		</tr>
		...
		<tr class="remove">
			...
			<?php foreach ( $products as $i => $product ) : $product_class = ( $i % 2 == 0 ? 'odd' : 'even' ) . ' product_' . $product->id ?>
				<td class="<?php echo $product_class; ?>">
					<a href="<?php echo add_query_arg( 'redirect', 'view', $this->remove_product_url( $product->id ) ) ?>"
					   data-product_id="<?php echo $product->id; ?>"><?php _e( 'Remove', 'ihosting' ) ?>
				...
				<?php foreach ( $products as $i => $product ) : $product_class = ( $i % 2 == 0 ? 'odd' : 'even' ) . ' product_' . $product->id; ?>
					<td class="<?php echo $product_class; ?>"><?php
						switch ( $field ) {
							...
							case 'image':
								$img_thumb = ihosting_resize_image( $product->fields[$field], null, $img_size_x, $img_size_y, true, true, false );
								$thumb_html = '<img class="kt-product-compare-img attachment-yith-woocompare-image size-' . esc_attr( $img_thumb['width'] ) . 'x' . esc_attr( $img_thumb['height'] ) . '" width="' . esc_attr( $img_thumb['width'] ) . '" height="' . esc_attr( $img_thumb['height'] ) . '" src="' . esc_url( $img_thumb['url'] ) . '" alt="' . ihosting_get_img_alt( $product->fields[$field] ) . '" />';
								echo '<div class="image-wrap">' . wptexturize( $thumb_html ) . '</div>';
								break;
							...
							case 'pa_color':
								$terms = wp_get_post_terms( $product->id, 'pa_color' );

								if ( !is_wp_error( $terms ) ) {
									if ( !empty( $terms ) ) {
										echo '<div class="product-colors-list-wrap">';
										foreach ( $terms as $term ) {
											$color = 'transparent';
											if ( function_exists( 'get_tax_meta' ) ) {
												$color = trim( get_tax_meta( $term->term_id, 'ihosting_color' ) ) != '' ? get_tax_meta( $term->term_id, 'ihosting_color' ) : 'transparent';
											}
											echo '<span class="compare-color" data-color="' . esc_attr( $color ) . '" style="background-color:' . esc_attr( $color ) . ' ;"></span>';
										}
										echo '</div><!-- /.product-colors-list-wrap -->';
									}
								}
								break;
