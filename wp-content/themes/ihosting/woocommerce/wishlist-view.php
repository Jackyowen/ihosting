	...
	<form id="yith-wcwl-form"
		...
		if ( !empty( $page_title ) ) :
			...
			<div
				...
				<?php echo apply_filters( 'yith_wcwl_wishlist_title', '<h2 class="simple-title no-link">' . $page_title . '</h2>' ); ?>
		...
		<table class="shop_table cart wishlist_table" data-pagination="<?php echo esc_attr( $pagination ) ?>"
			...
			<tr>
				...
				<th class="product-name">
					<span
						class="nobr"><?php echo apply_filters( 'yith_wcwl_wishlist_view_name_heading', __( 'Product Name', 'ihosting' ) ) ?></span>
				</th>
				...
				<?php if ( $show_price ) : ?>
                    ...
                    <span class="nobr">
                        <?php echo apply_filters( 'yith_wcwl_wishlist_view_price_heading', __( 'Unit Price', 'ihosting' ) ) ?>
                    </span>
					</th>
				...
				<?php if ( $show_stock_status ) : ?>
                    ...
                    <span class="nobr">
                        <?php echo apply_filters( 'yith_wcwl_wishlist_view_stock_heading', __( 'Stock Status', 'ihosting' ) ) ?>
                    </span>
					</th>
			...
			if ( count( $wishlist_items ) > 0 ) :
					...
					if ( $product !== false && $product->exists() ) :
						...
						<tr id="yith-wcwl-row-<?php echo $item['prod_id'] ?>"
								...
								<td class="product-remove">
									<div>
										<a href="<?php echo esc_url( add_query_arg( 'remove_from_wishlist', $item['prod_id'] ) ) ?>"
										   ...
										   title="<?php _e( 'Remove this product', 'ihosting' ) ?>">&times;</a>
									</div>
								</td>
							<?php endif; ?>
							...
							<td class="product-thumbnail">
								<?php
								$product_img_thumb = ihosting_resize_image( $product->get_image_id(), null, $img_size_x, $img_size_y, true, true, false );
								$product_img_thumb_html = '<img class="attachment-shop_thumbnail size-shop_thumbnail wp-post-image" width="' . esc_attr( $product_img_thumb['width'] ) . '" height="' . esc_attr( $product_img_thumb['height'] ) . '" src="' . esc_url( $product_img_thumb['url'] ) . '" alt="' . ihosting_get_img_alt( $product->get_image_id() ) . '" />';
								?>
								<a href="<?php echo esc_url( get_permalink( apply_filters( 'woocommerce_in_cart_product', $item['prod_id'] ) ) ) ?>">
									<?php echo wptexturize( $product_img_thumb_html ); ?>
								</a>
							</td>

							<td class="product-name">
								<a class="product-title" href="<?php echo esc_url( get_permalink( apply_filters( 'woocommerce_in_cart_product', $item['prod_id'] ) ) ) ?>"><?php echo apply_filters( 'woocommerce_in_cartproduct_obj_title', $product->get_title(), $product ) ?></a>
							...
							<?php if ( $show_price ) : ?>
								<td class="product-price">
									...
									elseif ( $product->price != '0' ) {
										echo wptexturize( $product->get_price_html() );
									}
									else {
										echo apply_filters( 'yith_free_text', __( 'Free!', 'ihosting' ) );
									}
									?>
								</td>
							<?php endif ?>
							...
							<?php if ( $show_stock_status ) : ?>
								<td class="product-stock-status">
									...
									if ( $stock_status == 'out-of-stock' ) {
										$stock_status = "Out";
										echo '<span class="wishlist-out-of-stock">' . __( 'Out of Stock', 'ihosting' ) . '</span>';
									}
									else {
										$stock_status = "In";
										echo '<span class="wishlist-in-stock">' . __( 'In Stock', 'ihosting' ) . '</span>';
									}
									?>
								</td>
							<?php endif ?>
							...
							<?php if ( $show_last_column ): ?>
								<td class="product-add-to-cart">
									...
									if ( $show_dateadded && isset( $item['dateadded'] ) ):
										echo '<span class="dateadded">' . sprintf( __( 'Added on : %s', 'ihosting' ), date_i18n( get_option( 'date_format' ), strtotime( $item['dateadded'] ) ) ) . '</span>';
									endif;
									?>
									...
									<?php if ( $available_multi_wishlist && is_user_logged_in() && count( $users_wishlists ) > 1 && $move_to_another_wishlist ): ?>
										<select class="change-wishlist selectBox">
											<option value=""><?php _e( 'Move', 'ihosting' ) ?></option>
											<?php
											foreach ( $users_wishlists as $wl ):
												...
												<option value="<?php echo esc_attr( $wl['wishlist_token'] ) ?>">
													...
													if ( $wl['wishlist_privacy'] == 1 ) {
														$wl_privacy = __( 'Shared', 'ihosting' );
													}
													elseif ( $wl['wishlist_privacy'] == 2 ) {
														$wl_privacy = __( 'Private', 'ihosting' );
													}
													else {
														$wl_privacy = __( 'Public', 'ihosting' );
													}
									...
									<?php if ( $is_user_owner && $repeat_remove_button ): ?>
										<a href="<?php echo esc_url( add_query_arg( 'remove_from_wishlist', $item['prod_id'] ) ) ?>"
										   ...
										   title="<?php _e( 'Remove this product', 'ihosting' ) ?>"><?php _e( 'Remove', 'ihosting' ) ?></a>
									<?php endif; ?>
								</td>
							<?php endif; ?>
						</tr>
				...
				<tr>
					<td colspan="<?php echo esc_attr( $column_count ) ?>"
					    class="wishlist-empty"><?php _e( 'No products were added to the wishlist', 'ihosting' ) ?></td>
				</tr>
				<?php
			endif;
					...
					<?php if ( $show_cb ) : ?>
						<div class="custom-add-to-cart-button-cotaniner">
							<a href="<?php echo esc_url( add_query_arg( array( 'wishlist_products_to_add_to_cart' => '', 'wishlist_token' => $wishlist_meta['wishlist_token'] ) ) ) ?>"
							   class="button alt"
							   id="custom_add_to_cart"><?php echo apply_filters( 'yith_wcwl_custom_add_to_cart_text', __( 'Add the selected products to the cart', 'ihosting' ) ) ?></a>
						</div>
					<?php endif; ?>
						...
						<div class="ask-an-estimate-button-container">
							<a href="<?php echo ( $additional_info ) ? '#ask_an_estimate_popup' : $ask_estimate_url ?>"
							   class="btn button ask-an-estimate-button" <?php echo ( $additional_info ) ? 'data-rel="prettyPhoto[ask_an_estimate]"' : '' ?> >
								...
								<?php echo apply_filters( 'yith_wcwl_ask_an_estimate_text', __( 'Ask for an estimate', 'ihosting' ) ) ?>
							</a>
						</div>
					<?php endif; ?>
	...
	<div id="ask_an_estimate_popup">
		<form action="<?php echo $ask_estimate_url ?>" method="post" class="wishlist-ask-an-estimate-popup">
			...
			<button class="btn button ask-an-estimate-button ask-an-estimate-button-popup">
				<?php echo apply_filters( 'yith_wcwl_ask_an_estimate_icon', '<i class="fa fa-shopping-cart"></i>' ) ?>
				<?php _e( 'Ask for an estimate', 'ihosting' ) ?>
			</button>
		</form>
	</div>
<?php endif; ?>