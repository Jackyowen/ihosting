...
class bridgeHttpRequest
{
	...
	function connect($url,$withHeaders=true,$withCookies=false) {
        ...
        if ((stristr($url, 'quantumvault') !== false || stristr($url, 'twocheckout')) && count($_GET) > 0) {
            $ignore = array('ccce');
            $get_params = array();
            foreach ($_GET as $k => $v) {
        ...
        #$this->debug(0, 'Call response: ' . print_r($data, true));
        ...
        if ($this->follow && isset ($headers['location']) && $headers['location']) {
            ...
            } else if (strstr($redir, 'cart.php?a=add&domain=register')) {
                $newRedir = cc_whmcs_bridge_parse_url($redir);
                header('location: '.$newRedir);
                die();
