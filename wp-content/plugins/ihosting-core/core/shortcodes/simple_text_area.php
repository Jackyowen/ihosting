...
add_action( 'vc_before_init', 'lkSimpleTextArea' );
function lkSimpleTextArea() {
	global $kt_vc_anim_effects_in;
	vc_map(
		array(
			'name'     => esc_html__( 'LK Simple Text Area', 'ihosting-core' ),
			'base'     => 'lk_simple_text_area', // shortcode
			'class'    => '',
			...
			'params'   => array(
				...
				array(
					...
					'holder'     => 'div',
					'class'      => '',
					'heading'    => esc_html__( 'Content', 'ihosting-core' ),
					'param_name' => 'content',
					'std'        => '',
				),
				array(
...
function lk_simple_text_area( $atts, $content = null ) {

	$atts = function_exists( 'vc_map_get_attributes' ) ? vc_map_get_attributes( 'lk_simple_text_area', $atts ) : $atts;
	...
	$css_class = 'simple-text-area-wrap wow ' . $css_animation . ' text-' . $text_align;
	...
	if ( trim( $content ) != '' ) {
		$content_html .= '<div class="lk-text-area" style="color: ' . esc_attr( $text_color ) . ';">';
		$content_html .= $content;
		$content_html .= '</div><!-- /.lk-text-area -->';
	}
...
add_shortcode( 'lk_simple_text_area', 'lk_simple_text_area' );
