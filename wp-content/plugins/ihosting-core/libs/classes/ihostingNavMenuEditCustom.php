 ...
 */
add_action( 'wp_update_nav_menu_item', 'ihosting_custom_nav_update', 10, 3 );
function ihosting_custom_nav_update( $menu_id, $menu_item_db_id, $args ) {
 ...
 */
add_filter( 'wp_setup_nav_menu_item', 'ihosting_custom_nav_item' );
function ihosting_custom_nav_item( $menu_item ) {
...
add_filter( 'wp_edit_nav_menu_walker', 'ihosting_custom_nav_edit_walker', 10, 2 );
function ihosting_custom_nav_edit_walker( $walker, $menu_id ) {
		...
		<div class="menu-item-settings" id="menu-item-settings-<?php echo $item_id; ?>">
			...
			<p class="field-icon-classes description description-thin">
				<label for="edit-menu-item-icon-classes-<?php echo $item_id; ?>">
					...
					<em class="desc"><?php esc_attr_e( 'Ex: fa fa-adjust', 'ihosting-core' ); ?></em>
				</label>
			</p>
			<div class="field-icon-img-url description description-wide">
				<label for="edit-menu-item-icon-img-url-<?php echo $item_id; ?>">
					...
					<input type="text" id="edit-menu-item-icon-img-url-<?php echo $item_id; ?>"
					       ...
					       placeholder="<?php esc_attr_e( 'Paste icon image url here', 'ihosting-core' ); ?>"/>
					<em class="desc"><?php esc_attr_e( 'Input icon image url or choose a default image below', 'ihosting-core' ); ?></em>
				</label>
				<span class="icon-img-default-wrap">
					<ul class="icon-img-list">
						<?php for ( $i = 12; $i <= 21; $i++ ) { ?>
							<li>
								<a href="#"
								   class="icon-img-url <?php echo $item->icon_img_url == IHOSTINGCORE_IMG_URL . 'menu-icons/' . $i . '.png' ? 'active' : ''; ?>">

									<img
										src="<?php echo esc_url( IHOSTINGCORE_IMG_URL . 'menu-icons/' . $i . '.png' ); ?>"
										alt="<?php echo $i; ?>"/>
			 ...
			 */
			if ( esc_attr( $item->object ) == 'megamenu' ):
				?>
				<p class="field-megamenu-custom description description-wide">
					<label for="edit-menu-item-mega-width<?php echo $item_id; ?>">
						<?php esc_attr_e( 'Mega menu width', 'ihosting-core' ); ?><br/>
				...
				<?php
			endif; // End if ( esc_attr( $item->object ) == 'megamenu' )
			/*
			 * end added field
			 */
			?>
