	...
	public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		if ( strcasecmp( $item->attr_title, 'divider' ) == 0 && $depth === 1 ) {
			...
			if ( trim( $item->icon_classes ) != '' ) {
				$item_output .= '<span class="zmb-social-icon item-icon item-font-icon ' . esc_attr( $item->icon_classes ) . '"></span>';
			}
			...
			if ( trim( $item->icon_img_url ) != '' ) {
				...
				$item_output .= '<img class="item-icon item-img-icon" src="' . esc_url( $item->icon_img_url ) . '" alt="' . esc_attr( $item->title ) . '" />';
			}
