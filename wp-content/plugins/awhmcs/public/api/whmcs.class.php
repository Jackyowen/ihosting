...
class WHMCS {
	...
	public function __construct(){
		...
		$handle = fopen(dirname(__FILE__).'/'.$settings_file, 'r') or die("Unable to open settings file!");
		$settings = explode(',',fgets($handle));		
		fclose($settings_file);

		$this->url = $settings[0];
		$this->username = $settings[1];
		$this->password = $settings[2];
		$this->accesskey = $settings[3];
	...
	public function getRenewals($uid){

		$params['uid'] = $uid;

		$response = $this->api("getrenewals", $params);
	...
	public function getDomainLock($domainId){
         ...
         */
	public function domainRenew($domain, $domainId){

        $params['domain'] = $domain;
        $params['domainid'] = $domainId;

	    $response = $this->api("domainrenew", $params);
	...
	public function resetpwd($email, $key, $password1, $password2){
	    ...
	    $response = $this->api("resetpwd", $params);
         ...
         * @link http://docs.whmcs.com/API:Get_Domain_WHOIS
         */
	public function getDomainWHOIS($domain, $domainId){
        $params['domainid'] = $domainId;
         ...
         * @return object
         * @throws WhmcsException
         * @link http://docs.whmcs.com/API:Register_Domain
         */
	public function checkDomain($domainName){
        $params['domain'] = $domainName;
        //$params['domainid'] = 1;
        
	    $response = $this->api("domainwhois", $params);
...
*/
	...
	public function transferDomain($domainName){
		
        $params["domain"] = $domainName;
        $params["eppcode"] = 123123;
        
	    $response = $this->api("domaintransfer", $params);
...
*/
	...
	public function getClientaddons($clientId){
		
		$action = "getclientsaddons";
		
		$response = $this->api($action, $clientId);
		
		return $response;
	}	

	public function getProducesaddons($clientId){
		
		$action = "getproductsaddons";
	...
	public function getDomainpricing(){
		...
		$response = $this->api("getdomainpricing");
		return $response;
	}

	public function getTldpricing(){

		$response = $this->api("gettldpricing");
	...
	public function getKnow(){
		...
		$response = $this->api("getknow");
		return $response;
	}	
	public function getPayment(){

		$response = $this->api("getpayment");
		return $response;
	}
	...
	public function getArknow(){

		$response = $this->api("getarticlesknow");
		return $response;
	}			

	public function getNetworks(){

		$response = $this->api("getnetworks");
		return $response;
	}	

	public function getDownloads(){

		$response = $this->api("getdownloads");
		return $response;
	}	

	public function getItemdownload(){

		$response = $this->api("getitemdownload");
		return $response;
	}	

	public function getDepartments(){
		$response = $this->api("getsupportdepartments");
		
		return $response;
	}	

	public function getTickets(){
		$response = $this->api("gettickets");
	...
	public function getTicket($ticketid){
		...
		$response = $this->api("getticket", $params);
		return $response;
	}		
	
	public function getPaymentmethods(){
		$response = $this->api("getpaymentmethods");
		
		return $response;
	}	
	
	public function getPricing($extension, $type){
		
		$params['extension'] = $extension;
		$params['type'] = $type;
		
		
		$response = $this->api("getpricing", $params);
	...
	public function getProductPricing($productid){
		...
		$response = $this->api("getproductpricing", $params);
		
		return $response;
	}
	
	public function getDomainfree($id){
		
		$params['productid'] = $id;
		
		$response = $this->api("getdomainfree", $params);
	...
	public function getServices($uid = 0, $serviceId = 0, $domain = '', $productId = 0, $serviceUsername = '', $start = 0, $limit = 0){
		...
		$response = $this->api("getservices", $params);
	...
	public function updatePassword($data){
		...
		$attributes = array("email","existingpw","newpw","confirmpw");

		foreach($attributes as $k){
			$client[$k] = $data->$k;
		}
		
		$response = $this->api("updatepass", $client);
	...
	public function updateContact($data){
		$attributes = array("contactid", "firstname", "lastname", "companyname", "email", "address1", "address2", "city", "state", "postcode", "country", "phonenumber", "password2", "permissions", "subaccount", "generalemails", "productemails", "domainemails", "invoiceemails", "supportemails", "skipvalidation");
		...
		if($contact['skipvalidation'] != true){
			if(!$contact['contactid'] || !$contact['firstname'] || !$contact['lastname'] || !$contact['email'] || !$contact['address1'] || !$contact['city'] || !$contact['state'] || !$contact['postcode'] || !$contact['country'] || !$contact['phonenumber'] || !$contact['password2']){
				//throw new WhmcsException("Required fields missing.");
				echo "Required fields missing.";
			}
		}
		
		$response = $this->api("updatecontact", $contact);
	...
	public function getContacts($email){
		$response = $this->api("getcontacts", $email);
	...
	public function getInvoice($invoiceid){
         ...
         */
	public function getInvoices($uid = 0, $status = '', $start, $limit){
		if($uid > 0){
			$params['userid'] = $uid;
		}
	...
	public function getInvoicesitem($uid = 0, $start, $limit){

		$response = $this->api("getinvoicesitem", array("userid" => $uid));
...
*/
	...
	public function updateInvoice($id){
	    ...
	    $response = $this->api('updateinvoice', $data);
	...
	public function checkout($data){
	    ...
	    $response = $this->api('checkout', $data);
	...
	public function acceptOrder($orderid, $serverid = null, $serviceusername = null, $servicepassword = null, $registrar = null, $autosetup = null, $sendregistrar = null, $sendemail = null){
     ...
     /**
     * Add Ticket
     * @param array $data Array with parameters, see {@link http://docs.whmcs.com/API:Add_Ticket_Reply}
     * @return object
     * @throws WhmcsException
     * @link http://docs.whmcs.com/API:Add_Client
     */
	public function openNewTicket($data){
		$attributes = array("clientid", "name", "email", "deptid", "subject", "message", "markdown", "priority", "contactid", "serviceid", "domainid", "customfields", "admin", "noemail");
	
		foreach($attributes as $k){
			$ticket[$k] = $data->$k;
		}
		
		$response = $this->api("openticket", $ticket);
     ...
     * @link http://docs.whmcs.com/API:Add_Client
     */
	public function ticketReply($data){
		$attributes = array("ticketid", "clientid", "name", "email", "message", "markdown", "status", "contactid", "adminusername");
	
		foreach($attributes as $k){
			$ticket[$k] = $data->$k;
		}
		
		$response = $this->api("addticketreply", $ticket);
		return $response;
	}	
	...
	public function getAnnouncements(){
		...
		$response = $this->api("getannouncements" ,$values, 'admin');
