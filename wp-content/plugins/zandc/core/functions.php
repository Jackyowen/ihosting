	...
	if ( !empty( $list_of_domains_to_check ) ) {
		...
		if ( $integrate_with == 'link' ) {
			...
			if ( trim( $integrate_link ) != '' ) {
				...
				$integrate_link_html = '<a href="' . esc_url( $integrate_link ) . '" target="_blank" class="zan-dc-integrate-link zan-dc-integrate-custom-link zan-dc-btn hover-overlay-light-to-dark-ltr">' . sanitize_text_field( $integrate_text ) . '</a>';
			}
		}
		...
		foreach ( $list_of_domains_to_check as $domain ) {
			...
			if ( $available->status == 0 ) {
				...
				$response['html'] .= '<p class="is-registered">' . balanceTags( $message ) . $whois_link_html . '</p>';
			}
			...
			elseif ( $available->status == 1 ) {
				...
				$response['html'] .= '<p class="not-registered">' . balanceTags( $message ) . $integrate_link_html . '</p>';
			}
			...
			elseif ( $available->status == 2 ) {
				...
				$response['html'] .= '<p class="no-whois-server">' . balanceTags( $message ) . '</p>';
			}
			$i++;
