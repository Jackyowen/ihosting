...
function zan_dc_core_enqueue_script() {
	...
	if ( isset( $zandc['zandc_transfer_link'] ) ) {
		if ( trim( $zandc['zandc_transfer_link'] ) != '' ) {
			$zandc_localize['transfer_link_html'] = '<a href="' . esc_attr( $zandc['zandc_transfer_link'] ) . '" data-domain="{domain}" class="zan-dc-transfer-link zan-dc-btn hover-overlay-light-to-dark-ltr">' . $transfer_text . '</a >';
		}
		else{
	...
	if ( $integrate_with == 'link' ) {
		...
		$integrate_link = zan_dc_get_option( 'zandc_integration_link', '' );
		if ( trim( $integrate_link ) != '' ) {
			$integrate_link_html = '<a href="' . esc_attr( $integrate_link ) . '" target="_blank" class="zan-dc-integrate-link zan-dc-integrate-custom-link zan-dc-btn hover-overlay-light-to-dark-ltr">' . sanitize_text_field( $integrate_text ) . '</a>';
			$zandc_localize['integrate_link_html'] = $integrate_link_html;
		}
	}
